...
ws['A1'].font = TITLE_FONT
ws['A2'] = '设计哲学：维度=退市通道，权重=历史退市贡献度，股东实力=调节变量。黄色列可修改。【老Z定稿 2026-08-28：C1面值10→6、B2审计8→12、C2壳价值规则反转(市值越低于壳价值分越高)、A2改扣非主营业务收入口径、F2重组8→6、A3扣非4→6，总分仍=100】'
...
V2 = [
    ['C1', '面值距离', '一、市场通道(交易类退市≈55%)', 6,
     '面值退市是强制退市第一大类型(2024年39家/全年52家)，不可逆、无整改机会',
     '≥5元:6 / 3~5:5 / 2~3:4 / 1.5~2:2 / 1.2~1.5:1 / <1.2:0（面值退市不可逆，但壳价值与审计对冲后属尾部事件，降权让位）', None, '老Z定稿2026-08-28：10→6，档位随分值重标'],
    ['C2', '壳价值锚定', '一、市场通道(交易类退市)', 8,
     '市值≈壳价值市场定价(大Deal-168壳费基准28亿)。壳有人接盘→不会跌破面值；退市公司市值中位数<10亿',
     '基准=平均壳价值28亿（壳交易市值中位数）：市值≤5折(≤14亿):8 / 5~7.5折:6 / 7.5折~1倍:4 / 1~1.5倍:2 / >1.5倍:0。市值低于壳价值越多→并购成本越低→买壳注资保壳概率越高；与C1对冲（壳有人接盘则难破面值）', None, '老Z定稿2026-08-28：8分不变，规则反转为『市值低于平均壳价值越多分越高』'],
    ...
    ['A2', '扣非主营业务收入', '三、财务红线（V1失真核心修正）', 12,
     '营收组合是财务类退市主触发，但"连续两年才退市"——第一年触线有18个月整改窗口；2024新规收紧"营业收入扣除"口径',
     '口径=扣除与主营无关收入后的主营业务收入（防贸易/垫资/卖资产冲量凑3亿）：达标:12 / 缺口≤20%:9 / 20~40%:6 / 40~60%:3 / >60%或连续两年触线:0；收入真实性存疑(贸易冲量)-3', None, '老Z定稿2026-08-28：营收→扣非主营业务收入口径（更严，剔除非主营水分）'],
    ['A3', '扣非盈利', '三、财务红线', 6,
     '扣非亏损是营收组合的伴生条件，本身不独立退市',
     '为正:6 / 亏损收窄:3 / 连亏3年+:0', None, '老Z定稿2026-08-28：4→6'],
    ...
    ['B2', '审计意见', '四、监管通道', 12,
     ...
     '标准无保留:12 / 带强调事项:9 / 保留意见:6 / 无法表示/否定:0（非标意见=规范类退市直接通道，提权至12分与S1并列第一权重）', None, '老Z定稿2026-08-28：8→12（审计意见是规范类退市直接通道，加权）'],
    ['F2', '重组/纾困进度', '五、纾困运作（反向对冲）', 6,
     '重整成功=最典型的退市反转路径(重整执行期公司几乎零退市)',
     '重整执行/法院批准:6 / 预重整受理:4 / 出售资产/债务豁免:2 / 无动作:0', None, '老Z定稿2026-08-28：8→6'],
    ['F1', '经营改善趋势', '五、纾困运作', 4,
...
r += 1
MOGAO = [
    ['C1/C2', '6+2', '5+8', '股价4.25元；市值13.65亿', 'C1收紧至6分制(4.25元→5)；C2规则反转：市值13.65亿≈壳基准28亿的5折→满8分(便宜=并购机会，正是莫高案例逻辑)'],
    ['S1/S2', '无+4(E1)', '10+6', '甘肃农垦28%持股，无质押', 'V2新增实控人性质：省级国资+订单/注资/资产注入能力'],
    ['A组', '2+2+7', '3+8+0', '营收2.04亿(缺口32%)；归母净资产7.07亿；扣非-0.89亿', 'A2改扣非主营收入口径(更严，缺口>32%→3分，防贸易冲量)；A1净资产厚度8分；扣非连亏A3提权后仍0分'],
    ['B组', '4+4+5', '10+12', '无立案；审计标准无保留', '无违规立案满分；B2提权12分制，标准无保留→12(干净审计是国资壳最大加分项)'],
    ['D1', '0', '0', '经营现金流-1.13亿', '两版一致，但V2权重10→4，伤害减半'],
    ['F组', '4+0', '2+0', '无重组公告动作', '国企背景不必然公告纾困（V2用S1体现而非F2）'],
    ['H1', '8', '4', '无冻结/限高/立案', '两版基本一致'],
    ['合计', '53分/B级/第171名', '≈68分/A级/前60名', '', 'B2/C2提权恰好对冲C1收紧与A2口径加严，总分仍≈68分/A级，与"退市风险很低"市场共识一致'],
]
for row in MOGAO:
...
# V1→V2 映射说明
r += 1
ws.cell(row=r, column=1, value='V1→V2 维度映射：E1+H1质押风险→S2+H1 | B1+B3→B1立案造假 | C2+G1→C2壳价值(老Z反转:越低于壳价值分越高) | A2改为扣非主营业务收入口径 | A3→A1 | A1→A3降权 | D1降权 | F1拆出趋势 | 新增S1 | B2加权至12(老Z) | C1降至6/F2降至6/A3升至6(老Z定稿)').font = Font(name='微软雅黑', size=9, color='808080')
...
r += 1
STRUCT = [
    ['交易类(面值退市)', '≈55%', '一、市场通道 14分(C1 6+C2 8)'],
    ['财务类', '≈32%', '三、财务红线 32分(A1 10+A2 12+A3 6+D1 4)'],
    ['规范类(非标审计/内控否定)', '≈8%', '四、监管通道 22分(B1 10+B2 12,含违法)'],
    ['重大违法(造假)', '≈5~9%', '四、监管通道'],
]
for row in STRUCT:
    put(ws, r, row, fill=(GRAY if r % 2 == 0 else None))
...
r += 1
FACTORS = [
    ['股价跌破面值', '核心(第一)', '交易类退市≈55%，不可逆', 'C1 6分(老Z降权)+壳价值C2 8分对冲'],
    ['扣非营收组合缺口', '核心(第二)', '财务类≈32%，主触发指标', 'A2 12分(老Z改为扣非主营业务收入口径)'],
    ['净资产为负', '核心', '财务类硬红线，连续两年退', 'A1 10分'],
    ['审计非标/内控否定', '核心(通道)', '规范类退市直接触发+财务类加速器', 'B2 12分(老Z提权)'],
    ['重大违法(造假)', '核心(独立通道)', '2025年造假退市创新高，零容忍', 'B1 10分'],
    ['实控人性质(国资)', '强调节变量', '国资退市占比<8%，但2024后非免死', 'S1 12分(新增)'],
    ['质押爆仓/控制权真空', '重要(传导)', '面值退市民企几乎全员', 'S2 6分'],
    ['壳价值(市值承接)', '重要', '退市公司市值中位数<10亿', 'C2 8分(老Z反转：越低于壳价值分越高)'],
    ['重整/纾困进度', '重要(反转)', '重整执行期公司几乎零退市', 'F2 6分(老Z降权)'],
