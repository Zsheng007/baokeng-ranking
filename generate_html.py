...
function applyDeepReport(code){{
  ...
  document.getElementById('expBody').innerHTML = `
    ...
    <div class="er-sec"><div class="er-sec-t">三、报告内容（V2十三维完整版）</div>
      <div class="er-sec-b">① 报告说明与口径 ② 评分总览（13维明细表）③ 逐维深度解读 ④ 维度画像：安全垫 vs 风险源 ⑤ 保壳逻辑（模型解读）⑥ 风险点清单 ⑦ 风险提示与研究局限。参考样本：《ST摸鱼风云-V2跟读报告_001_002883_ST中设_20260901.docx》。</div>
    </div>
...
<div class="modal-mask" id="memMask" onclick="if(event.target===this)closeModal('memMask')">
  <div class="modal">
    <div class="modal-head"><div class="modal-title">🔐 会员专享 · ST摸鱼风云-V2</div><button class="modal-close" onclick="closeModal('memMask')">✕</button></div>
    <div class="modal-body" id="memBody"></div>
  </div>
</div>
<div class="modal-mask" id="expMask" onclick="if(event.target===this)closeModal('expMask')">
  <div class="modal">
    <div class="modal-head"><div class="modal-title" id="expTitle">🧠 ST摸鱼风云-V2 · 即时跟读报告</div><button class="modal-close" onclick="closeModal('expMask')">✕</button></div>
