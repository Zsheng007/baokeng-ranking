...
## ST摸鱼风云-V5 报告模块 + 短信验证3篇（2026-09-02 老Z需求，方案文档阶段）
- 需求：评分榜单保留 + 跟读报告模块换成「ST摸鱼风云-V5深度报告」+ 手机号短信验证 + 免费看3篇
- **V5 已定型**：ST摸鱼风云-V5 = V2 的定型升级版（2026-09-02老Z定案），评分仍用 score_v2 十三维（榜单=报告分数不变）；模板引擎 `~/.workbuddy/skills/baokeng-rank/scripts/st_moyu_fengyun_v5_report_template.py`（配置驱动，CFG/S/各章data）
- **V5模板现状**：CFG/评分表/跟踪指标/免责声明完整，但**第1-7章正文是占位文本**——完整报告需逐股票填充正文（实施阶段，非自动）
- **短信成本**：验证码 0.045元/条起（阿里云2026-05-20后0.045-0.05元），千客户约百元级可忽略
- **短信合规**：工信部实名制，个人账号普通短信签名难报备→个人推荐「阿里云短信认证」（码自动回填）或注册企业/个体户
- **后端铁律**：验证码必须服务端发（前端泄密钥）；3篇额度必须服务端记（清缓存可绕过）→纯前端只能测试
- 交付物：`ST摸鱼风云V5报告模块_短信验证3篇_设计方案.docx`（10章+附录，成本/架构三方案/额度模型/分阶段实施）+ `ST摸鱼风云-V5_ST冀凯_002691_报告.docx`（V5参考样例）
- 老Z决策：只要方案文档、只升TOP10为V5、短信服务商未定（待其选）
- 实施路径：阶段一升V5报告库→阶段二加回页面报告模块→阶段三接短信后端（认证+部署）；推荐路径为老Z参考
